...
@app.route('/api/external/send-follow-up-data', methods=['POST'])
@require_api_key
def external_send_follow_up_data():
    """
    Fetches all follow-up emails and sends them to a webhook,
    intended for external automation use with an API key.
    """
    try:
        conn = database.get_db_connection()
        emails = conn.execute('SELECT id, name, content, outlet_name, city FROM follow_up_emails').fetchall()
        ...
        print(f"Error in external_send_follow_up_data: {e}")
        return jsonify({"error": "An internal server error occurred."}), 500
